...
namespace Plot.Services
{
    ...
    public class TokenService
    {
        ...
        // Stores token lifetime from settings.
        ...
        /// Gets settings from an IOptions instance.
        /// </summary>
        /// <param name="tokenSettings"> IOptions instance with settings using 
        /// the tokenSettings model </param>
        /// <exception cref="ArgumentNullException">Thrown if settings are 
        /// missing</exception>
        public TokenService(IOptions<TokenSettings> tokenSettings) 
            {
                _issuer=tokenSettings.Value.Issuer ?? throw new 
                    ArgumentNullException(tokenSettings.Value.Issuer);

                _audience=tokenSettings.Value.Audience ?? throw new 
                    ArgumentNullException(tokenSettings.Value.Audience);
        ...
        {
            //Create a claims arry to store the email claim.
            var claims = new Claim[]
            {
                // Create a new email claim with the user's email.
                new(ClaimTypes.Email, userEmail)
            };

            // Convert applications secret key into a byte array
            // to create a new symmetric security key.
            var key = new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes(_secretKey));

            // Create signing credentials using the key and HMAC-SHA256
            // algorithm.
            var signingCredentials = new SigningCredentials(
                key, SecurityAlgorithms.HmacSha256);

            // Start time for the tokens expiration lifetime.
            //Use UTC time to prevent time zone issues.
            var tokenStartTime = DateTime.UtcNow;

            // Add the lifetime to the start time to get the expiration time.
            var tokenExpirationTime = tokenStartTime.AddMinutes(_lifeTime);

            //Create a new JwtSecurityToken
            var token = new JwtSecurityToken(
                _issuer, // Issuer in settings
                _audience, // Audience in settings
                claims, // Claims array with email claim
                tokenStartTime, // Start time
                tokenExpirationTime, // Expiration time
                signingCredentials); // Signing credentials based on the secret
                                     // key.
